...
Dependency model mirrors Python's extras: the core has **zero runtime dependencies**. `make ts-install` installs **dev tooling only** (Biome, tsup, tsx, TypeScript, Vitest) — every package in `devDependencies` supports Node ≥18, so the CI matrix (Node 18/20/22) installs cleanly. All provider/framework SDKs are **optional `peerDependencies`** and are NOT auto-installed (`auto-install-peers=false` in `libraries/typescript/.npmrc`); install them on demand: `make ts-install-base` for the LLM provider SDKs + LangChain glue (the `examples` extra analog), and `make ts-install-{langgraph,langchain,deepagents,claude-agent-sdk,openai-agents,vercel-ai}` for a framework — each runs `pnpm add -D` (the `uv sync --extra <name>` analog; reset with `git checkout -- package.json pnpm-lock.yaml && make ts-install`). The SDKs are kept out of the base install because some (e.g. `@aws-sdk/client-bedrock-runtime`, `@langchain/*`) declare `engines.node >=20`. Because wrappers/integrations type provider clients structurally (`any`) and never hard-import the SDK type packages — and the one dynamic import (`@openai/agents` in the openai-agents adapter) is indirected through a `string` specifier — typecheck/test/build all pass without any SDK installed.
