"""DeepAgents — multi-turn agent with sessions.

Builds a deep agent and runs multiple conversation turns under a single
``session_id`` using ``pandaprobe.session()``. The callback handler
...
Run:
    uv run python examples/deepagents/03_multi_turn.py
"""
...
agent = create_deep_agent(
    ...
    system_prompt=(
        "You are a helpful travel advisor. Plan briefly with write_todos when "
        "the user's request has multiple parts. Keep answers to 2-3 sentences. "
        "Use the get_attraction tool when the user asks about specific cities."
    ),
)


def chat_turn(user_message: str, history: list, handler: DeepAgentsCallbackHandler) -> str:
